...
def test_check_density(test_dir):
    result, _, nr_segments = check_density_file(os.path.join(test_dir, "data/linestrings.json"), 100)
    assert not result, "expected result is False"
    assert nr_segments > 0


def test_check_density_gc(test_dir):
    result, _, nr_segments = check_density_file(os.path.join(test_dir, "data/fc-geometry-collection.json"), 100)
    assert not result, "expected result is False"
    assert nr_segments > 0
...
def test_check_density_not_pass(linestring_feature_gj):
    ...
    result: Nested[ReportLineString] = transform_geojson_geometries(feature, _check_density_geometry)
...
def test_densify_file_exception(linestring_3d_feature_gj):
    ...
    with pytest.raises(
        ...
        _: Nested[ReportLineString] = transform_geojson_geometries(feature, _check_density_geometry)
